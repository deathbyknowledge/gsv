...
pub enum TimeoutStage {
    ...
    Write,
    #[error("waiting for Desktop")]
    Handler,
}

#[derive(Debug, thiserror::Error)]
pub enum Error {
    #[error("GSV Desktop is already running")]
    AlreadyRunning,
    #[error("unsafe Desktop control endpoint: {0}")]
    UnsafeEndpoint(EndpointSafety),
    #[error("Desktop control peer is not the current user")]
    PeerIdentity,
    #[error("Desktop control frame is empty")]
    EmptyFrame,
    #[error("Desktop control frame is {actual} bytes; maximum is {maximum}")]
    FrameTooLarge { actual: usize, maximum: usize },
    #[error("malformed Desktop control frame")]
    MalformedFrame(#[source] serde_json::Error),
    #[error("Desktop control protocol version {actual} is unsupported; expected {expected}")]
    UnsupportedVersion { actual: u16, expected: u16 },
    #[error("Desktop control response did not match its request")]
    UnexpectedResponse,
    #[error("Desktop control peer disconnected before the operation completed")]
    PeerDisconnected,
    #[error("Desktop control peer sent more than one request on a connection")]
    UnexpectedClientData,
    #[error("Desktop control operation timed out while {stage} after {duration:?}")]
    Timeout {
        stage: TimeoutStage,
        duration: Duration,
    },
    #[error("Desktop rejected the request: {0:?}")]
    Remote(ErrorCode),
    #[error("Desktop control I/O failed")]
    Io(#[source] io::Error),
}
