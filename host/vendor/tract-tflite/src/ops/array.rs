...
use tract_core::ops::array::{Gather, MultiBroadcastTo, Slice, TypedConcat};
...
pub fn register_all(reg: &mut Registry) {
    ...
    reg.reg_to_tract(BuiltinOperator::GATHER, de_gather);
...
fn de_gather(op: &mut DeserOp) -> TractResult<TVec<OutletId>> {
    let options = builtin!(op, builtin_options_as_gather_options);
    ensure!(options.batch_dims() == 0, "GATHER batch_dims is not supported");
    let rank = op.ctx.target.outlet_fact(op.inputs[0])?.rank();
    let axis = if options.axis() < 0 {
        rank as i32 + options.axis()
    } else {
        options.axis()
    };
    ensure!((0..rank as i32).contains(&axis), "GATHER axis is out of range");
    let indices = wire_cast(
        format!("{}.indices", op.prefix),
        op.ctx.target,
        &op.inputs[1..2],
        i64::datum_type(),
    )?;
    op.ctx.target.wire_node(
        op.prefix,
        Gather::new(axis as usize),
        &[op.inputs[0], indices[0]],
    )
}
...
fn de_unpack(op: &mut DeserOp) -> TractResult<TVec<OutletId>> {
    ...
    let axis = if options.axis() < 0 {
        input.rank() as i32 + options.axis()
    } else {
        options.axis()
    };
    ensure!((0..input.rank() as i32).contains(&axis), "UNPACK axis is out of range");
    let axis = axis as usize;
    ensure!(
        input.shape[axis].to_i64()? == options.num() as i64,
        "UNPACK output count does not match its axis"
    );
    let mut outputs = tvec!();
    for index in 0..options.num() as usize {
