...
mod tests {
    ...
    #[test]
    fn launch_mode_defaults_headless_and_accepts_debug_opt_in_exactly() {
        assert_eq!(
        ...
        assert_eq!(launch_mode(None, None), Some(LaunchMode::Headless));
        assert_eq!(launch_mode(Some(OsStr::new("true")), None), None);
        assert_eq!(
            launch_mode(None, Some(OsStr::new("0"))),
            Some(LaunchMode::Headless)
        );
    ...
    #[test]
    fn explicit_disable_stops_gestures_unless_debug_was_requested() {
        assert_eq!(launch_mode(Some(OsStr::new("0")), None), None);
        assert_eq!(
            launch_mode(Some(OsStr::new("0")), Some(OsStr::new("1"))),
            Some(LaunchMode::Debug)
        );
    }
    ...
    fn resolution_prefers_override_then_sibling_then_workspace_target() {
        ...
        let manifest = workspace.join("host/apps/desktop");
        let installed = workspace.join("installed");
        fs::create_dir_all(&manifest).expect("Desktop directory");
    ...
    fn stalled_ui_receives_the_latest_fresh_status_and_every_reliable_event() {
        ...
        send_vision_event(
            ...
            VisionEvent::Intent {
                ...
                intent: GestureIntent::VoiceRequest {
                    voice_request_id: 31,
                    action: gesture_protocol::VoiceRequestGestureIntent::Mute,
                },
            },
        )
