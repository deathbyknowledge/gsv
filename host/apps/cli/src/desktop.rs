...
fn desktop_is_starting(error: &Error) -> bool {
    desktop_is_absent(error)
        || matches!(
            error,
            Error::Timeout {
                stage: desktop_protocol::TimeoutStage::Connect,
                ..
            }
        )
}
...
fn legacy_microphone_name_matches(
    devices: &[desktop_protocol::MicrophoneDevice],
...
mod tests {
    ...
    use desktop_protocol::MicrophoneDevice;
    ...
    fn only_transport_absence_triggers_a_desktop_launch() {
        ...
        assert!(!desktop_is_absent(&Error::UnsupportedVersion {
            actual: 1,
            expected: desktop_protocol::PROTOCOL_VERSION,
        }));
        ...
        assert!(!desktop_is_starting(&Error::Timeout {
            stage: desktop_protocol::TimeoutStage::Read,
            duration: Duration::from_millis(1),
        }));
    }
