...
export function describeUpstreamPull(
  ...
  if (record && record.upstreamChanged === false && record.localChanged === false) {
...
function selectPullRecord(value: unknown): UpstreamPullRecord | null {
  ...
  if (imports) {
    const records = imports.map(asRecord).filter((item): item is Record<string, unknown> => item !== null);
    return normalizePullRecord(records.find((item) => item.diverged === true) ?? records[0] ?? null);
  }
