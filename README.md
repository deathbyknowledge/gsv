...
- Reach it from anywhere — web UI, CLI, or an extensible adapter system with
  WhatsApp, Discord, Telegram, and Slack implementations bundled today.
...
**Prerequisites:** a [Cloudflare account](https://dash.cloudflare.com/sign-up), plus any credentials required by your chosen model provider. The baseline deployment supports Workers Free; Workers Paid is optional for CodeMode and additional runtime capacity. Model usage is billed separately by its provider.
...
press `Tab` to switch between Ship and a literal shell on that target, type `?`
for the compact key guide, or launch with `gsv tui --vim` for opt-in Vim browse
controls. Up/Down recalls commands, Page Up/Down scrolls the transcript, and
Escape leaves the prompt visible while browsing media or earlier output.
Commands and replies remain in one scrollable terminal document. The
line-oriented `gsv chat "..."` command remains available for scripts.
