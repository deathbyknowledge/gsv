...
Use this guide when you want to connect a local machine to GSV so agents can use
its filesystem and shell.
...
If `gsv auth setup --node-id ...` already issued a token, skip this step.
...
`node.*` because those are the persisted driver fields.

## Use Devices From an Agent

Agents do not receive a separate device-only tool set. They use the normal tools
and choose a `target`.
...
Each device should have a stable id and ideally a token bound to that id:
...
Then configure each machine locally and install the device daemon there.
...
- [Manage Processes](./manage-processes.md)
- [Hardware Tools](../reference/hardware-tools.md)
