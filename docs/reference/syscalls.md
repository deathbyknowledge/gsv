...
| `proc.spawn` | `handleProcSpawn` | Resolves the run-as identity (Master Control for a parentless interactive default, the personal agent for parentless background work, the parent for an inherited child, or explicit `runAs`), registers a process, sends kernel-only `proc.setidentity`, and optionally admits the initial prompt. |
