...
|---|---|---|
| `fs.read` | `handleFsRead`; CLI `Read` | Resolves paths against process `cwd` and home. Direct directory results are JSON. A successful file result always attaches a response body containing raw UTF-8 text or image bytes; `data` contains file metadata. Text decoding is strict across native and device implementations, so invalid UTF-8 returns a binary-file error. `offset` defaults to `0`; `limit` defaults to all lines. Agent tool results add line numbers when presenting text to the model. Native image transport adds no filesystem-specific cap; target implementations may enforce their advertised storage policy. Android caps files at 64 MiB and direct text reads at 8 MiB. Process tool results cap model-context materialization at 25 MiB. |
