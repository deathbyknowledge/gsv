...
- Gateway-to-adapter delivery uses canonical request, response, and signal frames. Routed `message.committed` and `proc.run.hil.requested` signals are durably accepted by the adapter before acknowledgement; the adapter then owns provider presentation, retry, ambiguity handling, outcome reporting, and body cleanup.
