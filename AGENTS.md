...
Contact Conversations have no Process handler. Pairing and incoming social messages, offers, or revocation do not by themselves admit agent work. Explicit local work offers and acceptance create commitments; later remote updates continue to advance existing responsibilities. Automatic social assistance requires its own scoped admission, and an attention-policy migration must preserve existing work and disclose the change to the owner.

A scoped Process starts with fresh history and an immutable Kernel-owned grant. Every descendant inherits that grant and its shared allowances, independently of run-as changes. Scope checks precede routing and remain authoritative after approval, revocation, expiry and Process deletion. Its filesystem contains only selected immutable materials; private standing context, account archive homes, targets and integrations do not enter by inheritance. Existing shipped runtime context remains available without copying deployment or account overrides. See `docs/architecture/process-scopes.md`.

Optional automatic social help feeds the ordinary Process queue from a bounded Kernel-owned admission queue. Only newly committed, attributed human messages in the explicitly granted conversation may trigger it. Admission preserves an exact cause and stable event identity; automatic replies share finite allowances and one response per admitted message. Remote Process messages and delivery/control events never restart the exchange. The direct human chooses access and renewal; an incoming message cannot enlarge either.
