...
export class WhatsAppInstallation extends DurableObject<ManagedWhatsAppPeerEnv> {
  private readonly owner: AdapterInstallationRetirement;
  constructor(ctx: DurableObjectState, env: ManagedWhatsAppPeerEnv) {
    super(ctx, env);
    if (!ctx.id.name) throw new Error("WhatsApp installation identity is unavailable");
...
export class WhatsAppLifecycleEntrypoint extends WorkerEntrypoint<ManagedWhatsAppPeerEnv, { authority?: string }> {
  private service() {
    return new AdapterLifecycle({ authority: this.ctx.props?.authority, directory: this.env.ACCOUNTS,
      coordinator: (id) => this.env.WHATSAPP_INSTALLATIONS.getByName(id),
      coordinatorId: (id) => this.env.WHATSAPP_INSTALLATIONS.idFromName(id).toString(),
      namespace: (kind) => kind === "adapter-peer" ? this.env.MANAGED_WHATSAPP_PEER : kind === "adapter-pairing" ? this.env.MANAGED_WHATSAPP_PAIRING : null,
    });
  }
...
function resolveResource(env: ManagedWhatsAppPeerEnv, resource: AdapterInstallationResource) {
  const namespace = resource.kind === "adapter-peer" ? env.MANAGED_WHATSAPP_PEER : resource.kind === "adapter-pairing" ? env.MANAGED_WHATSAPP_PAIRING : null;
  if (!namespace) return null;
  if (namespace.idFromName(resource.name).toString() !== resource.objectId) throw new Error("WhatsApp resource identity mismatch");
