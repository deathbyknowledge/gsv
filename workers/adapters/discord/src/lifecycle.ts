...
export class DiscordInstallation extends DurableObject<SharedDiscordEnv> {
  private readonly owner: AdapterInstallationRetirement;
  constructor(ctx: DurableObjectState, env: SharedDiscordEnv) {
    super(ctx, env);
    if (!ctx.id.name) throw new Error("Discord installation identity is unavailable");
...
export class DiscordLifecycleEntrypoint extends WorkerEntrypoint<SharedDiscordEnv, { authority?: string }> {
  private service() {
