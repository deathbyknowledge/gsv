...
import { ProcessScopeAutomation } from "./process-scope-automation";
...
export class ProcessScopeStore {
  readonly automation: ProcessScopeAutomation;
  constructor(private readonly sql: SqlStorage) { this.automation = new ProcessScopeAutomation(sql); }
  ...
  create(ownerUid: number, rootPid: string, policy: ProcessScopePolicy, now = Date.now()): ProcessScope {
    ...
    if (parsed.automatic) {
      ...
      if (parsed.conversations.length !== 1 || !conversation.read || conversation.send !== (parsed.automatic.mode === "reply")
        || (parsed.automatic.mode === "draft" ? parsed.budgets.messages !== 0 : parsed.budgets.messages < 1 || parsed.budgets.messages > parsed.automatic.maxMessages)) {
        throw new Error("Automatic help requires one readable conversation and a matching finite reply allowance");
      }
    }
    this.prune(now);
  ...
  get(id: string, now = Date.now()): ProcessScope | null {
    ...
    return row ? {
      ...
      automation: this.automation.status(id),
