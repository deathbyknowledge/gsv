...
describe("federation version negotiation", () => {
  ...
  it("authenticates feature advertisement against the pinned identity without changing v1", async () => {
    await runWithRealKernelSql(async (_sql, storage) => {
      ...
      expect(setProtocol).toHaveBeenCalledWith(contact.id, contact.generation, expect.objectContaining({ version: 2, features: ["messages", "approaches", "work", "context"] }));
  ...
  it("falls back only for a missing endpoint and never downgrades an established v2 peer", async () => {
    await runWithRealKernelSql(async (_sql, storage) => {
      ...
      await expect(negotiateContactProtocol({ ...contact, protocol: { version: 2, features: ["messages", "approaches", "work", "context"], checkedAtMs: 0 } }, context))
  ...
  it("accepts an empty POST stream for discovery and rejects a nonempty body", async () => {
    await runWithRealKernelSql(async (_sql, storage) => {
      ...
      expect(await discovered.json()).toMatchObject({ version: 2, features: ["messages", "approaches", "work", "context"] });
