...
import { createProcessAiConfigSnapshot, isProcessAiConfigKey, redactProcessAiConfigSnapshot } from "./ai-config";
...
export class ProcessSettingsService {
  ...
  get aiTextGenerateConfig(): AiTextGenerateConfig | undefined {
    const snapshot = this.host.store.state.getAiConfigSnapshot();
  ...
  getAiConfig(args: ProcAiConfigGetArgs): ProcAiConfigGetResult {
    const snapshot = this.host.store.state.getAiConfigSnapshot();
    return {
      ok: true,
      pid: this.host.pid,
  ...
  async setAiConfig(args: ProcAiConfigSetArgs): Promise<ProcAiConfigSetResult> {
    ...
    if ("clear" in args) {
      ...
      const current = this.host.store.state.getAiConfigSnapshot();
    ...
    if (snapshot && (Object.keys(snapshot.values).length > 0 || snapshot.profile)) {
      this.host.store.state.setAiConfigSnapshot(snapshot);
    } else {
      snapshot = null;
      this.host.store.state.clearAiConfigSnapshot();
    }
