...
import type { AssistantMessage, Context, ToolResultMessage, UserMessage } from "@earendil-works/pi-ai";
import {
...
function validateCompactionRequest(
  ...
  if (hasKeepLast === hasThroughMessageId) {
    return compactionFailure(
      "proc.history.compact requires exactly one of keepLast or throughMessageId",
    );
  }
...
export class ProcessHistory {
  ...
  private captureCompactionSnapshot(
    ...
    const selected = this.host.store.history.getHistoryPrefixMessages({
