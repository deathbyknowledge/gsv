...
describe("process history", () => {
  ...
  it("selects manual compaction history by token pressure instead of message count", async () => {
    ...
    await runInProcess(stub, (process) => {
      const store = process.store;
      store.messages.appendMessage("user", `large old turn ${"x".repeat(20_000)}`);
      for (let index = 0; index < 5; index += 1) {
        store.messages.appendMessage(
          index % 2 === 0 ? "assistant" : "user",
          `small recent turn ${index}`,
        );
      }
      const stats = store.messages.messageStats();
      store.state.setContextState(buildProcContextState({
        revision: store.state.nextContextStateRevision(),
