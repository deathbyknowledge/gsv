import type {
  ...
  InstallationState,
} from "@humansandmachines/gsv/services/directory";
import {
  hostnameForHandle,
  installationIdentity,
  normalizeHostname,
...
export type InstallationReservation = InstallationIdentity & {
...
export class AccountStore {
      ...
      await this.db.batch([
        ...
        this.db.prepare(
          `INSERT INTO memberships (
             installation_id, principal_id, state, created_at
           ) VALUES (?, ?, 'pending', ?)`,
  ...
  async resetInstallation(input: {
    ...
    const participants = [...new Set((input.participants ?? []).map((id) =>
      parseOpaqueId(id, "reset participant"),
    ))];
    ...
    const results = await this.db.batch([
      ...
      ...participants.map((participant) => this.db.prepare(
        `INSERT INTO installation_reset_participants (
           operation_id, participant_id, state, updated_at
         )
         SELECT operation_id, ?, 'pending', ?
         FROM installation_reset_operations
         WHERE operation_id = ? AND replacement_installation_id = ?`,
      ).bind(participant, now, operationId, replacement.installationId)),
    ]);
  ...
  async beginProvisioning(
    ...
    await this.requireResetPrepared(reservation.installationId);
    if (
  ...
  private async requireResetPrepared(installationId: string): Promise<void> {
    const pending = await this.db.prepare(
      `SELECT 1 AS pending FROM installation_reset_operations r
       JOIN installation_reset_participants p ON p.operation_id = r.operation_id
       WHERE r.replacement_installation_id = ? AND p.state = 'pending'
       LIMIT 1`,
    ).bind(installationId).first();
    if (pending) throw new Error("installation reset preparation is pending");
  }
}
