...
function RoutineEditorForm({ original, onSaved, onCancel, onDirty, timezone }: RoutineEditorProps & { timezone: string }) {
  ...
  return <form class="fleet-process-form fleet-work-form" aria-label={original ? "Edit routine" : "New routine"} onSubmit={(event) => { event.preventDefault(); if (!save.isPending && connected) save.mutate(); }}>
    ...
    <fieldset disabled={save.isPending || !connected}>
      ...
      <label>Repeat<select value={draft.cadence} onChange={(event) => {
        // SAFETY: This select contains exactly the four RoutineDraft cadence values below.
        field("cadence", event.currentTarget.value as typeof draft.cadence);
      }}><option value="daily">Every day</option><option value="weekly">Every week</option><option value="every">At an interval</option><option value="cron">Custom schedule</option></select></label>
