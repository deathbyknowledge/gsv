...
People is the dedicated communication surface; its design contract is [People in Instrument](../../../../../engineering/social-web-design.md). The contact inspector separates relationship details, canonical messages and cross-Ship requests. Messages support older history, text and attachments; each contact’s draft and send identity remain in the owning surface while selecting other rows. Leaving People or reloading protects unsent drafts; uploads are cancelled on teardown. Active conversation and request caches refresh from their exact owner-scoped signals, and hidden views wait until opened. Request actions retain revision checks and show human-readable states and structured details. Revoked contacts keep readable history with sending disabled. Delivery acceptance is labelled separately from confirmed delivery.
