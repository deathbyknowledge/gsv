...
describe("saved messenger connections", () => {
  it("loads discovery and only the signed-in person's saved identities", async () => {
    const call = vi.fn<GSVClient["call"]>().mockImplementation(async (method) => {
      if (method === "adapter.list") return { adapters: [{ adapter: "telegram", available: true, supportsPairing: true, accounts: [] }] };
  ...
  it("distinguishes a saved identity from the transport being ready", () => {
    ...
    const adapter = { adapter: "telegram", available: true, supportsConnect: false, supportsDisconnect: false, supportsSend: true, supportsStatus: true, supportsActivity: true, supportsPairing: true, accounts: [account] };
