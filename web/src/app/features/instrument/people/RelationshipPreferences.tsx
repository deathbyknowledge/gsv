...
export function RelationshipPreferences({ contact, account }: { contact: ContactSummary; account: ConsoleAccount | undefined }) {
  ...
  return <section class="people-relationship" aria-label="Private relationship preferences">
    ...
    {preferences && <>
      ...
      <label class="people-setting"><input type="checkbox" checked={preferences.shipAttention === true} disabled={disabled} onChange={(event) => update.mutate({ shipAttention: event.currentTarget.checked })} /><span>Let Ship receive new messages<small>When enabled, a new message can wake Ship and give it this conversation to handle. It never sends a reply on its own.</small></span></label>
      <label class="people-notifications">Message notifications<select disabled={disabled || preferences.muted} value={preferences.notifications} onChange={(event) => { const value = event.currentTarget.value; if (value === "notify" || value === "digest" || value === "quiet") update.mutate({ notifications: value }); }}><option value="notify">Notify me</option><option value="digest">Daily digest</option><option value="quiet">Quiet</option></select></label>
      <p class="note">A digest gathers messages here for 24 hours before alerting you. Changing notification preferences clears queued alerts; unread messages stay in your inbox.</p>
      <p class="note">These preferences are private. Receiving a message and granting Ship permission to send are separate decisions.</p>
    </>}
