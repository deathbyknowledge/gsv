...
function PrivateHelper({ helper, contact, account, onReply, onOpenWork }: {
  ...
  return <section class="people-private-helper" aria-label="Private helper replies">
    ...
    {grant && <>
      <div class="people-helper-heading"><span class="people-kicker">{!active ? "Access ended" : !allowance ? "Allowance used" : helper.state === "idle" ? "Private helper" : helper.state === "waiting_hil" ? "Waiting for your decision" : "Working privately"}</span><button class="people-action" onClick={() => onOpenWork(helper.pid)}>open work and continue →</button></div>
...
function HelperAccess({ scope }: { scope: ProcessScope }) {
  ...
  </dl>{scope.policy.materials.map((material) => {
