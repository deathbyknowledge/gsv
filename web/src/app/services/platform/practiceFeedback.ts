...
const detected: Record<PracticeGesture, string> = {
...
const retry: Record<PracticeTarget, string> = {
...
const poses: Partial<Record<GestureCandidate, PracticeGesture>> = {
...
const expectedPoses: Partial<Record<PracticeTarget, PracticeGesture>> = {
...
export function practiceHold(snapshot: NativeSnapshot, target: PracticeTarget): string | null {
  ...
  const pose = poses[snapshot.gesture_progress.candidate];
  if (!pose) return null;
  if (pose === "both_fists") return "Hold to turn hands-free off";
  if (pose !== expectedPoses[target]) return `${detected[pose]} detected · this step: ${retry[target]}`;
