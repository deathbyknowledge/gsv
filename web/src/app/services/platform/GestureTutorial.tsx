...
export function GestureTutorial({ scope, onClose }: { scope: string; onClose(): void }) {
  ...
  const entered = useRef({ sequence: 0, actions, gesture: false, hadDraft: false });
  ...
  const go = (next: number) => {
    entered.current = { sequence: snapshot?.gesture_action_sequence ?? 0, actions, gesture: false, hadDraft: Boolean(value.current.trim()) };
    setComplete(false);
    setStep(next);
  };
  useEffect(() => {
    ...
    if (snapshot.gesture_action_sequence > entered.current.sequence && snapshot.gesture_action === lesson.action) entered.current.gesture = true;
    const accepted = entered.current.gesture;
    const done = step === 1 ? accepted && listening
      : step === 2 ? listening && Boolean(draft.trim())
      ...
      : step === 7 ? Boolean(snapshot.scroll_velocity)
  ...
  return <dialog ref={dialog} class="native-tutorial" aria-labelledby="native-tutorial-title" data-instrument-dialog
    ...
    <div class="native-tutorial-body">
      <section class="native-tutorial-visual" aria-label="Gesture demonstration">
        <GestureIllustration lesson={lesson.lesson} label={lesson.instruction} />
        ...
        <div class="native-tutorial-feedback" role="status">
          <span>{notice ? "Input needs attention" : !snapshot ? "Connecting input…" : !camera && !voice ? "Camera and microphone off" : preparing && voice ? "Preparing microphone…" : !camera && listening ? "Listening · camera off" : feedback?.progress != null ? feedback.message : feedback?.action ?? feedback?.message}</span>
          <progress max={1000} value={feedback?.progress ?? 0} aria-label="Gesture hold" />
        </div>
      </section>
      <section class="native-tutorial-lesson">
        ...
        {step === 0 ? <>
          ...
          <p class="native-panel-footnote">Start practice enables the camera. A gesture starts the microphone. Closing this guide stops both.</p>
          <InputSoundSettings />
        </> : <>
        ...
        {complete && <p class="native-tutorial-success" role="status">✓ {step === 8 ? "Camera and microphone off. You’re ready." : "Done. Make a fist before the next command."}</p>}
      </section>
    </div>
    <footer class="native-tutorial-footer">
      ...
      <div>
        {step === 0 ? <>
          ...
          <button type="button" class="native-primary" disabled={!complete} onClick={() => go(step + 1)}>continue →</button>
        </>}
      </div>
    </footer>
  </dialog>;
