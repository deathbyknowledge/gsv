            ...
            conn.set_frame_handler(move |frame| match frame {
                Frame::Req(req) => {
                    ...
                    let request_body = req
                        ...
                        .transpose()
                        .map_err(|error: gsv_client::BodyError| error.to_string());
...
mod tests {
    ...
    async fn pending_body_error(call: &str, args: serde_json::Value) -> String {
        let channel =
            gsv_client::BinaryBodyChannel::new(BinaryBodyLimits::default(), |_frame| async {
                Ok(())
            })
            .unwrap();
        let body = channel
            .receive(gsv_client::FrameBodyDescriptor {
                stream_id: 41,
                length: Some(1),
            })
            .unwrap();
