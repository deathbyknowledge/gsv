...
export const GSV_PROCESS_ORCHESTRATION =
  ...
  "\n" +
  "When a scheduled event and any resulting reply should be visible in the current process conversation's matching Web chat, use `sched add --here --name <name> (--every <duration> | --cron <expr> [--timezone <zone>] | --after <duration> | --at <timestamp>) --message <prompt> [--conversation <id>]`. `--at` requires a future ISO timestamp with `Z` or an explicit numeric UTC offset. This creates a typed process event for the current process and its active conversation; it does not create a detached child. The schedule is bound to that process id, so recreate it after killing the process. `--here` does not preserve an external adapter route; recurring adapter delivery must explicitly send through that adapter target. A successful firing records event admission, not completion of a model turn or reply. Examples: `sched add --here --name animal-facts --every 2m --message 'Send one obscure animal fact.'`, `sched add --here --name daily-brief --cron '0 9 * * *' --timezone Europe/Amsterdam --message 'Prepare and send the daily brief.'`.\n" +
  "\n" +
  "Use `crontab` and cron files only for recurring shell-command or true fire-and-forget background automation. `crontab -l` lists the current user's cron table, `crontab FILE` installs one, and `/var/spool/cron/<username>` is the editable per-user file. Each job is a five-field cron line followed by a shell command. The crontab file is desired state; reinstalling it regenerates the linked Kernel schedule ids. Example: `printf '0 9 * * * proc spawn --non-interactive --label refresh-index \"Refresh the search index.\"\\n' > ~/jobs.cron && crontab ~/jobs.cron`. By default, a scheduled `proc spawn` runs as your personal agent in its own process; `--as` or `--parent` can select a different run-as identity. Its answer stays in that child's history. A schedule status of `ok` for spawned work means dispatch and spawn acceptance, not child completion or delivery.\n" +
  "\n" +
