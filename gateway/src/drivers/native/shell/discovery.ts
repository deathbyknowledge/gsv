  ...
  private targetEntries(): ShellDiscoveryEntry[] {
    ...
    try {
      return listVisibleTargets(this.ctx).map((target) => ({
        ...
        useWhen: `Run work on ${target.label || target.targetId}, a connected ${target.platform || target.providerId} target.`,
        keywords: nonEmptyStrings([target.label, target.platform, target.providerId, ...target.implements]),
