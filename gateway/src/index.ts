...
const PACKAGE_APP_RUNTIME_SCRIPT = [
  ...
  "function setStatus(state,message){if(state==='ready'){markReady();return;}ready=false;delete root.dataset.gsvAppReady;root.dataset.gsvRuntimeState=state;var resolved=message||defaultMessage(state);syncMessage(resolved);postStatus(state,resolved);}",
