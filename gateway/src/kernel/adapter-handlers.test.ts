...
describe("adapter lifecycle handlers", () => {
  ...
  it("returns home immediately while a selected work process is still running", async () => {
    ...
    sendFrameToProcessMock.mockImplementation(async (_pid: string, frame: any) => ({
      ...
      id: frame.id,
      ok: true,
      data: { runId: frame.args.eventId, queued: false },
    }));
    ...
    expect(sendFrameToProcessMock).toHaveBeenCalledWith(personal.processId, expect.objectContaining({
  ...
  it("retries a failed work-return event against the current personal controller", async () => {
    ...
    expect(sendFrameToProcessMock.mock.calls[1]?.[1].args.eventId)
      .toBe(sendFrameToProcessMock.mock.calls[0]?.[1].args.eventId);
