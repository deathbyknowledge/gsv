...
async function ensureColdPersonalController(
  ...
  if (current) {
    ...
    try {
      await initializePersonalController(
        ctx.installationId,
        current.processId,
        identity,
      );
...
async function spawnPersonalController(
  ...
  try {
    await initializePersonalController(ctx.installationId, pid, identity);
  } catch (error) {
    try {
...
async function initializePersonalController(
  ...
  const response = await sendFrameToProcess(
    ...
    pid,
    request,
  ) as ResponseFrame<"proc.setidentity"> | null;
...
async function rollbackPersonalController(
  ...
  const response = await sendFrameToProcess(
    ...
    pid,
    request,
  ) as ResponseFrame<"proc.kill"> | null;
  if (!response || response.type !== "res" || response.id !== request.id) {
