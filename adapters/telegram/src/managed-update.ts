...
/**
 * Normalizes only private, human-authored Telegram messages. Group, channel,
 * bot, and unrelated update types are deliberately ignored before a peer DO is
 * selected. Media and other rich content are retained only as an explicit
 * unsupported-content bit; no provider bytes or file identifiers are stored.
 */
export function normalizeManagedTelegramUpdate(
  ...
  if (!actorId || !surfaceId || !messageId || actorId !== surfaceId) {
    return { kind: "ignored" };
  }

  const updateId = nonNegativeIntegerString(update.update_id);
...
export function isManagedTelegramConnectCommand(text: string): boolean {
  const command = text.trim().split(/\s+/, 1)[0]?.toLowerCase();
  return command === "/connect" || command === "/link";
}
...
function displayName(first: unknown, last: unknown): string | undefined {
  const parts = [first, last]
    ...
    .filter(Boolean);
  const name = parts.join(" ").slice(0, MAX_DISPLAY_NAME_LENGTH);
