...
| Presence/voice | `app/features/presence/*` | recorder state, VAD, run signal buffering, transcription, speech synthesis/playback, preferences | Controller now delegates browser audio, speech text, speech output, signal parsing, timing, preferences, and display helpers. Next split: recorder/transcription and run-activity controllers or hooks. |
