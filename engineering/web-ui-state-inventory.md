...
| Presence/voice | `app/features/presence/presenceControl.ts` | recorder state, VAD, run signal buffering, transcription, speech synthesis/playback, preferences | Split into `features/presence` hooks. Use mutations for `ai.transcription.create` and `ai.speech.create`; keep media/audio/run-stream state local. |
